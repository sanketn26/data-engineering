...
It was **B**. It works, and it fights the engine the whole way — which is the
...
and session gaps have to be evaluated per user rather than per batch. Structured
Streaming can be made to do all of it. Every one of those is native in the
engine built for event time, and a workaround in the engine built for batches.
...
the right one — and the cluster already running the lake keeps running the lake.
